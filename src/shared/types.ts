...
export interface PaneNode {
  ...
  /**
   * Per-pane AI session id for restore-by-id, shown/edited in the pane's id popover.
   * Each pane resumes its OWN conversation (claude --resume / codex resume <id>) instead
   * of the latest-in-dir. Works for local AND ssh (the resume runs wherever the pane does).
   * - claudeSessionId: a UUID lmux mints for a claude pane (claude --session-id then --resume).
   * - codexSessionId: codex can't be told an id, so the user pastes the one codex generated.
   * Empty on shell panes and on AI panes not yet given an id (those keep their plain command).
   */
